...
class TorIptables:
  ...
  def __init__(self):
    self.iface = getoutput("ip route show default | awk '/default/ {print $5}'")
    self.local_dnsport = "53"
    self.tor_dnsport = "9053"  # DNSPort
    self.virtual_net = "10.254.0.0/16"  # VirtualAddrNetwork
    ...
    self.tor_config_file = '/etc/torrc.d/custom.conf'
    self.torrc = r'''
...
''' % (basename(__file__), self.trans_port, self.virtual_net,
  ...
  def load_iptables_rules(self):
    ...
    call(["iptables", "-t", "nat", "-A", "OUTPUT", "-p", "udp", "--dport",
          self.local_dnsport, "-j", "REDIRECT", "--to-ports", self.tor_dnsport])
    # custom
    call(["iptables", "-t", "nat", "-A", "PREROUTING", "-i", self.iface, "-p", "tcp", "-d", self.virtual_net,
         "-j", "REDIRECT", "--to-ports", self.trans_port])
    call(["iptables", "-t", "nat", "-A", "OUTPUT", "-p", "tcp", "-d", self.virtual_net,
         "-j", "REDIRECT", "--to-ports", self.trans_port])
